...
Nếu addon không cài được, kiểm tra manifest URL và chứng chỉ HTTPS. Nếu catalog trống, kiểm tra `src/data/media.json`, deploy lại và làm mới addon trong Nuvio. Nếu có item nhưng không có stream, gọi trực tiếp stream endpoint và kiểm tra `PUBLIC_BASE_URL`. Nếu `curl` trả 403, lấy URL đã ký mới hoặc kiểm tra signing secret; với 404 hoặc 502, kiểm tra Drive file ID, quyền tệp, OAuth credentials và Drive API. Nếu `curl` trả 206 nhưng Nuvio không phát được, thử MP4 chuẩn H.264/AAC và đổi cài đặt trình phát hoặc decoder của Nuvio; vấn đề có thể nằm ở codec hoặc trình phát. Không chia sẻ URL đã ký hay secret khi gửi thông tin chẩn đoán.
