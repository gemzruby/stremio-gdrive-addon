...
If the addon does not install, check the manifest URL and HTTPS certificate. If the catalog is empty, check `src/data/media.json`, redeploy, and refresh the addon in Nuvio. If the item appears but the stream is absent, test the stream endpoint directly and check `PUBLIC_BASE_URL`. If video requests return `502` with `GOOGLE_AUTH_FAILED`, test the refresh token with `make preview-media`; Google's `invalid_grant` means the token is no longer usable. Obtain a new refresh token with your own OAuth client credentials in OAuth Playground, update `.dev.vars`, then run `make preview-media && make deploy`. You do not need to regenerate media if the folder contents have not changed. OAuth Playground can revoke tokens after 24 hours unless you use your own application credentials; an External OAuth app in Testing can issue refresh tokens that expire after seven days. These are possible causes, not a diagnosis of any particular token. If `curl` returns `403`, obtain a fresh signed URL or check the signing secret; if it returns `404` or another `502`, check the Drive file ID, file permissions, OAuth credentials, and Drive API availability. If `curl` returns `206` but Nuvio cannot play, try the baseline MP4 and another Nuvio player/decoder setting; that points to player or codec compatibility rather than the addon API. Do not send signed URLs or secret values when sharing diagnostics.
